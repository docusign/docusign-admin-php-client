...
namespace DocuSign\Admin\Client\Auth;

/**
 * Oauth Class Doc Comment
 *
 * @category    Class
 * @package     DocuSign\Admin\Client\Auth
 * @author      DocuSign
 */
class OAuth
{
    ...
    public function setOAuthBasePath($oAuthBasePath = null)
    {
        ...
        if (substr($this->getBasePath(), 0, 13) === "https://api-d"
            || substr($this->getBasePath(), 0, 12) === "http://api-d"
        ) {
            $this->oAuthBasePath = self::$DEMO_OAUTH_BASE_PATH;
        } elseif (substr($this->getBasePath(), 0, 13) === "https://api-s"
            || substr($this->getBasePath(), 0, 12) === "http://api-s"
        ) {
